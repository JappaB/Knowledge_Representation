...
def difference(clues, solution):
    ...
    for key in solution.keys():
        if clues[key] != solution[key]:
            difference[key] = solution[key]
        else:
            difference[key] = '0'
...
if __name__ == '__main__':
    ...
    for i, grid in enumerate(sudoku_lines):
        ...
        new_clues = pick_additional_clues(possible_tranfers, 10)
        new_sudoku = clue_values.copy()

        for key in new_clues.keys():
            new_sudoku[key] = new_clues[key]

        # print(display_string(new_sudoku))
        print(display_string(search(clue_values)))
